...
> - **What lives inside?** When IIS installs, it puts a default file named `iisstart.htm` inside `wwwroot`.
> - **How it works:** When a client opens `http://server1.e6.local` in a browser, IIS opens `C:\inetpub\wwwroot\iisstart.htm` and displays the blue IIS Welcome Page.
> - **Custom Websites:** When a company deploys its own website, web developers replace `iisstart.htm` inside `wwwroot` with their custom `index.html` and website code!
