...
> **Understanding the `C:\inetpub\` Directory Structure:**
> When you install Web Server (IIS) and FTP, Windows creates `C:\inetpub\` containing these standard system folders:
>
> | Folder | Purpose | What lives inside? |
> |:---|:---|:---|
> | **`ftproot`** | **FTP Storage Root** | Files uploaded & downloaded via FTP (`ftp://server1.e6.local`). |
> | **`wwwroot`** | **Website Storage Root** | HTML/Web files served by Web Server (`http://server1.e6.local`). |
> | **`custerr`** | **Custom Errors** | HTML pages for HTTP error codes (`404 Not Found`, `500 Server Error`). |
> | **`logs`** | **IIS & FTP Activity Logs** | Log files recording client IP addresses, connection times, and requests. |
> | **`history`** | **Configuration Backup** | Automatic backup copies of IIS settings (`applicationHost.config`). |
> | **`temp`** | **Temporary Cache** | Compiled ASP.NET code and cached data generated by IIS. |
